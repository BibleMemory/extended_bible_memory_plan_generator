...
- **ESV Crossref ready:** verse references render with abbreviated book names ("Eph. 1:1") and the widget dispatches `esv-crossref.trigger-linkify` after each render, so if the host page loads Crossway's [ESV Cross-Reference Tool](https://www.esv.org/resources/esv-crossreference-tool/), every reference becomes a hover-popup link. Date cells are excluded from scanning (a "Mar. 15" date would otherwise link as Mark 15). The PDF keeps compact chapter:verse references. Note: the tool validates against ESV verse counts, so the few KJV-only verses (e.g. 2 Corinthians 13:14) stay unlinked.
- **Shareable links:** generating a plan writes the selection into the URL (`?bmp-book=Ephesians&bmp-chapter=3&bmp-start=2026-07-26&bmp-vpd=1&bmp-dpw=6`), and opening such a link prefills the form and renders the identical plan immediately. Invalid values fall back to defaults; unrelated query parameters are preserved.
